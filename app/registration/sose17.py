...
EXKURSIONEN_TYPES = {
    ...
    "stad": (
        "Klassische Stadtführung mit Reichstagsbesichtigung",
        25,
        "Klass. Stadtf.",
    ),
...
def registration_sose17_report_rahmenprogramm():
    ...
    return render_template(
        "admin/sose17/rahmenprogramm.html",
...
def registration_sose17_report_spitznamen():
    ...
    return render_template("admin/sose17/spitznamen.html", result=result)
...
def registration_sose17_report_orgaprobleme():
    ...
    for reg in registrations:
        if reg.data["orgaprobleme"]:
            result.append(reg)
    return render_template("admin/sose17/orgaprobleme.html", result=result)


@registration_blueprint.route(
    "/admin/registration/<int:reg_id>/details_sose17", methods=["GET", "POST"]
)
@groups_sufficient("admin", "orga")
def registration_sose17_details_registration(reg_id):
    ...
    if form.validate_on_submit():
        ...
        if old_spitzname != form.spitzname.data:
            return redirect(
                url_for("registration.registration_sose17_report_spitznamen")
            )
        elif old_overwrite and old_overwrite != form.exkursion_overwrite.data:
            return redirect(
                url_for("registration.registration_sose17_report_exkursionen")
            )
        else:
            return redirect(
                url_for(
                    "registration.registration_sose17_details_registration",
                    reg_id=reg_id,
                )
            )
    if "exkursion_overwrite" in reg.data:
