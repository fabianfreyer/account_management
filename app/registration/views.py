...
def unis():
    ...
    return render_template(
        "admin/unis.html", unis=unis, unis_registrations=unis_registrations
    )
...
@groups_sufficient("admin", "orga")
@confirm(title="Delete university", action="Delete", back="registration.unis")
...
def registrations():
    ...
    return render_template(
        "admin/registrations.html", registrations=registrations, uni=None
    )
...
@registration_blueprint.route(
    "/admin/registration/<int:reg_id>/delete", methods=["GET", "POST"]
)
@groups_sufficient("admin", "orga")
@confirm(
...
def registrations_export_teilnehmer_csv():
    ...
    writer.writerows(
        [
            [reg.user.full_name, reg.uni.name]
            for reg in registrations
            if reg.is_zapf_attendee
        ]
    )
...
def registrations_export_attendee_csv():
    registrations = [
        reg
        for reg in Registration.query.order_by(Registration.uni_id).all()
        if reg.is_zapf_attendee
    ]
    result = io.StringIO()
    writer = csv.writer(result, quoting=csv.QUOTE_NONNUMERIC)
...
def mascots():
    ...
    return render_template("admin/mascots.html", mascots=mascots, uni=None)
...
@registration_blueprint.route(
    "/admin/mascots/<int:masc_id>/delete", methods=["GET", "POST"]
)
@groups_sufficient("admin", "orga")
@confirm(title="Delete mascot", action="Delete mascot", back="registration.mascots")
...
def mascot_export_json():
    ...
    return jsonify(
        mascots=[{"name": masc.username, "uni_name": masc.uni.name} for masc in mascots]
    )
...
def mascots_export_csv():
    ...
    result = io.StringIO()
    writer = csv.writer(result, quoting=csv.QUOTE_NONNUMERIC)
    writer.writerows([[masc.name, reg.uni.name] for masc in mascots])
    return Response(result.getvalue(), mimetype="text/csv")
