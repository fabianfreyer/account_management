...
EXKURSIONEN_TYPES_FORM = [("nooverwrite", "")] + [
    (name, data[0]) for name, data in EXKURSIONEN_TYPES.items()
]
...
ESSEN_TYPES = {"vegetarisch": "Vegetarisch", "vegan": "Vegan", "omnivor": "Omnivor"}

HEISSE_GETRAENKE_TYPES = {"egal": "Egal", "kaffee": "Kaffee", "tee": "Tee"}
...
MITTAG2_TYPES = {"vegan": "Sojaschnitzel mit Nudeln", "normal": "Hähnchen mit Nudeln"}

MITTAG3_TYPES = {"vegan": "veg. Currywurst", "normal": "Currywurst"}
...
class Winter17ExkursionenOverwriteForm(FlaskForm):
    ...
    exkursion_overwrite = SelectField(
        "Exkursionen Festlegung", choices=EXKURSIONEN_TYPES_FORM
    )
    submit = SubmitField()
...
def wise17_calculate_exkursionen(registrations):
    ...
    regs_overwritten = [
        ...
        and reg.data["exkursion_overwrite"] != "nooverwrite"
    ]
    regs_normal = sorted(
        [
            reg
            for reg in registrations
            if not ("exkursion_overwrite" in reg.data)
            or reg.data["exkursion_overwrite"] == "nooverwrite"
        ],
        key=get_sort_key,
    )
        ...
        if not got_slot:
            for field_index, field in enumerate(EXKURSIONEN_FIELD_NAMES):
                ...
                if not result[exkursion_selected]:
                    return None
                if (
                    result[exkursion_selected]["space"] == -1
                    or result[exkursion_selected]["free"] > 0
                ):
                    result[exkursion_selected]["registrations"].append(
                        (reg, field_index)
                    )
                    result[exkursion_selected]["free"] -= 1
                    got_slot = True
                    break
        if not got_slot:
            result["nospace"]["registrations"].append(
                (reg, len(EXKURSIONEN_FIELD_NAMES) + 1)
            )
...
def registration_wise17_report_exkursionen():
    registrations = [
        reg
        for reg in Registration.query.order_by(Registration.id)
        if reg.is_zapf_attendee
    ]
    result = wise17_calculate_exkursionen(registrations)
    return render_template(
        "admin/wise17/exkursionen.html",
        result=result,
...
def registration_wise17_report_essen():
    ...
    for reg in registrations:
        ...
        if (
            not result_essen[essen_type]
            or not result_heisse_getraenke[heisse_getraenke]
            or not result_mittag1[mittag1_type]
            or not result_mittag2[mittag2_type]
            or not result_mittag3[mittag3_type]
        ):
            return None
...
def registration_wise17_report_rahmenprogramm():
    ...
    return render_template(
        "admin/wise17/rahmenprogramm.html",
...
def registration_wise17_report_spitznamen():
    ...
    return render_template("admin/wise17/spitznamen.html", result=result)


@registration_blueprint.route(
    "/admin/registration/<int:reg_id>/details_wise17", methods=["GET", "POST"]
)
@groups_sufficient("admin", "orga")
def registration_wise17_details_registration(reg_id):
    ...
    if form.validate_on_submit():
        ...
        if old_spitzname != form.spitzname.data:
            return redirect(
                url_for("registration.registration_wise17_report_spitznamen")
            )
        elif old_overwrite != form.exkursion_overwrite.data:
            return redirect(
                url_for("registration.registration_wise17_report_exkursionen")
            )
        else:
            return redirect(
                url_for(
                    "registration.registration_wise17_details_registration",
                    reg_id=reg_id,
                )
            )
    if "exkursion_overwrite" in reg.data:
...
def registration_wise17_export_stimmkarten_latex():
    ...
    for uni in unis:
        uni_regs = [
            reg
            for reg in Registration.query.filter_by(uni_id=uni.id)
            if reg.is_zapf_attendee
        ]
        if len(uni_regs) > 0:
...
def registration_wise17_export_idkarten_latex():
    result = [
        "\\idcard{{{}}}{{{}}}{{{}}}".format(reg.id, reg.user.full_name, reg.uni.name)
        for reg in Registration.query.order_by(Registration.uni_id)
        if reg.is_zapf_attendee
    ]
    return Response("\n".join(result), mimetype="application/x-latex")
    ...
    for name, data in result_exkursionen.items():
        for reg in data["registrations"]:
            ...
            result.append(
                (
                    reg[0].uni_id,
                    "{type}{{{spitzname}}}{{{name}}}{{{uni}}}{{{id}}}{{{exkursion}}}".format(
                        type=ausweis_type,
...
def registration_wise17_export_strichlisten_latex():
    ...
    for reg in registrations:
        ...
        result.append(
            "{} & {} & \\\\[0.25cm] \\hline".format(reg.user.full_name, reg.uni.name)
        )
        i += 1
        if i == 34:
            result.append("}")
...
def registration_wise17_export_bmbflisten_latex():
    ...
    for reg in registrations:
        ...
        result.append(
            "{} & {} & {} &&& \\\\[0.255cm] \\hline".format(
                lfd_nr, reg.user.full_name, reg.uni.name
            )
        )
        i += 1
        lfd_nr += 1
...
@registration_blueprint.route(
    "/admin/registration/report/wise17/taschentassenlisten/latex"
)
@groups_sufficient("admin", "orga")
...
@registration_blueprint.route(
    "/admin/registration/report/wise17/ausweisidbeitraglisten/latex"
)
@groups_sufficient("admin", "orga")
...
def registration_wise17_export_t_shirt_latex():
    ...
    for reg in registrations:
        ...
        result.append(
            "{} & {} & {} \\\\[0.25cm] \\hline".format(
                reg.user.full_name, reg.uni.name, reg.data["tshirt"]
            )
        )
        i += 1
        if i == 34:
...
def registrations_wise17_export_wichteln_csv():
    ...
    writer.writerows(
        [
            [reg.user.full_name, reg.uni.name, reg.data["spitzname"]]
            for reg in registrations
            if reg.is_zapf_attendee
        ]
    )
...
def registration_wise17_export_exkursionen_latex():
    ...
    for name, data in result_exkursionen.items():
        buffer = [
            "\\exkursionspage{{{type}}}{{".format(type=EXKURSIONEN_TYPES[name][2])
        ]
        for reg in data["registrations"]:
            buffer.append(
                "{} & {} \\\\ \\hline".format(reg[0].user.full_name, reg[0].uni.name)
            )
        buffer.append("}")
        result.append("\n".join(buffer))
    return Response("\n".join(result), mimetype="application/x-latex")
...
def registration_wise17_export_bestaetigungen_latex():
    ...
    for reg in registrations:
        result.append(
            "\\bestaetigung{{{}}}{{{}}}".format(reg.user.full_name, reg.uni.name)
        )
    return Response("\n".join(result), mimetype="application/x-latex")
...
def registrations_wise17_export_id_name_csv():
    ...
    writer.writerows(
        [
            [reg.id, "{} ({})".format(reg.user.full_name, reg.uni.name)]
            for reg in registrations
            if reg.is_zapf_attendee
        ]
    )
